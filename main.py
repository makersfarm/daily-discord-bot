...
def _trigger_github_actions(payloads: list[dict]) -> None:
    ...
    resp = requests.post(
        ...
        json={"ref": "main", "inputs": {"payload": json.dumps(payloads)}},
        timeout=15,
    )
...
def main():
    ...
    payloads = discord.build_payloads(pub_data, hub_data, pub_idea, hub_idea)
    PAYLOAD_PATH.write_text(json.dumps(payloads, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"💾 payloads({len(payloads)}건) 저장: {PAYLOAD_PATH}")

    try:
        discord.send(pub_data, hub_data, pub_idea, hub_idea)
        print(f"📨 Discord 직접 전송 성공 ({len(payloads)}건)")
    except Exception as e:
        print(f"⚠️ Discord 직접 전송 실패: {e}")
        try:
            _trigger_github_actions(payloads)
